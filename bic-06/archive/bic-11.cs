...
public class BicForm: Form {
    ...
    // IRC color/formatting code regex - removes ^Cnn,nn, bold, italic, underline, etc.
    private static readonly Regex ircCodesRegex = new Regex(
        @"[\x02\x0F\x16\x1F\x1D]|\x03(?:\d{1,2}(?:,\d{1,2})?)?",
    ...
    private void ProcessIRCMessage(string rawMessage) {
        ...
        foreach (string line in lines) {
            ...
            // Parse PRIVMSG for nicer display (with color codes stripped)
            if (line.Contains(" PRIVMSG ")) {
                ...
                if (colonIndex > 0) {
                    ...
                    string msg = ircCodesRegex.Replace(line.Substring(colonIndex + 1).Trim(), "");
            ...
            // Handle RPL_LISTSTART (322) - beginning of channel list
            if (line.Contains(" 322 ")) {
                string[] parts = line.Split(' ');
                string channel = parts[3];
                string userCount = parts[4];
                string topic = string.Join(" ", parts, 5, parts.Length - 5).TrimStart(':');
                topic = ircCodesRegex.Replace(topic, "");
                
                SendText($"[{userCount}] {channel}: {topic}\r\n", Color.Magenta);
            }
            ...
            if (line.Contains(" 323 ")) {
                ...
                SendText($"*** End of LIST ({total} channels)\r\n", Color.Yellow);
            }
        }
    }
    ...
    private void ProcessCommand(string input) {
        ...
        switch (cmd) {
            ...
            case "part":
                ...
                break;
            case "list":
                SendText("*** Fetching channel list...\r\n", Color.Yellow);
                SendRaw("LIST");
                break;
            case "msg":
            case "notice":
